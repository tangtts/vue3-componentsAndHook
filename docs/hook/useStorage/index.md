# useLocaleStorage
> 对 localStorage 进行操作，包括 设置`setStorage`,获取单个`getStorage`,获取所有`getAllStorage`,判断是否有`hasStorage`  删除`removeStorage`等
...
<script setup>
import useStorage from "../../components/useStorage/index.vue"
</script>
